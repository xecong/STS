...
public enum CardAnimationType
{
    Quick,      // 빠른 속도 (기본 카드용)
    Standard,   // 표준 속도
    Enhanced,   // 강화 카드용 (약간의 딜레이/이펙트 포함)
    Core        // 핵심 카드용 (긴 딜레이와 화려한 이펙트)
}
...
public partial class CardSequenceHandler : MonoBehaviour
{
    ...
    // 애니메이션 타입 결정
    private CardAnimationType DetermineCardAnimationType(Card card)
    {
    ...
    private IEnumerator PlayCardWithTiming(CardUI cardUI, Card card)
    {
